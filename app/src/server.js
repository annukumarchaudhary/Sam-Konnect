...
const hostUsersString = process.env.HOST_USERS || '[{"username": "Sam Konnect", "password": "P2P"}]';
...
app.post([`${apiBasePath}/token`], (req, res) => {
    ...
    if (!api.isAuthorized()) {
        log.debug('Sam Konnect get token - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('Sam Konnect get token - Authorized', {
        header: req.headers,
        body: req.body,
        token: token,
    });
...
app.get([`${apiBasePath}/meetings`], (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('Sam Konnect get meetings - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    // log.debug the output if all done
    log.debug('Sam Konnect get meetings - Authorized', {
        header: req.headers,
        body: req.body,
        meetings: meetings,
    });
...
app.post([`${apiBasePath}/meeting`], (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('Sam Konnect get meeting - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    log.debug('Sam Konnect get meeting - Authorized', {
        header: req.headers,
        body: req.body,
        meeting: meetingURL,
    });
...
app.post([`${apiBasePath}/join`], (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('Sam Konnect get join - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    log.debug('Sam Konnect get join - Authorized', {
        header: req.headers,
        body: req.body,
        join: joinURL,
    });
